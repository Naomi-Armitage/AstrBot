...
export const useExtensionPage = () => {
  ...
  const openUpdateConfirmDialog = (extensionName, forceUpdate = false) => {
    ...
    const defaultRepoUrl = String(ext?.repo || "").trim();
    const currentSource = String(ext?.update_repo_url || ext?.repo || "").trim();
    updateConfirmDialog.extensionName = extensionName;
    updateConfirmDialog.forceUpdate = forceUpdate;
    updateConfirmDialog.defaultRepoUrl = defaultRepoUrl;
    updateConfirmDialog.repoUrl = currentSource;
    updateConfirmDialog.hadCustomUpdateSource = !!ext?.has_custom_update_source;
    updateConfirmDialog.persistUpdateSource = !!ext?.has_custom_update_source;
  ...
  const closeUpdateConfirmDialog = () => {
    ...
    updateConfirmDialog.repoUrl = "";
    updateConfirmDialog.defaultRepoUrl = "";
    updateConfirmDialog.hadCustomUpdateSource = false;
    updateConfirmDialog.persistUpdateSource = false;
  };
  ...
  const confirmUpdatePlugin = async () => {
    ...
    const repoUrlInput = String(updateConfirmDialog.repoUrl || "").trim();
    const defaultRepoUrl = String(updateConfirmDialog.defaultRepoUrl || "").trim();
    const hadCustomUpdateSource = !!updateConfirmDialog.hadCustomUpdateSource;
    const persistUpdateSource = !!updateConfirmDialog.persistUpdateSource;
    const usesCustomSource = !!repoUrlInput && repoUrlInput !== defaultRepoUrl;
    const downloadUrl = usesCustomSource ? "" : getUpdateDownloadUrl(ext);
    closeUpdateConfirmDialog();
    ...
    try {
      const res = await axios.post("/api/plugin/update", {
        ...
        repo_url: usesCustomSource ? repoUrlInput : "",
        persist_update_source: usesCustomSource && persistUpdateSource,
        clear_persisted_update_source: !usesCustomSource && hadCustomUpdateSource,
      });
